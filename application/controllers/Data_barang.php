...
public function ajax_barang()
{
    ...
    if ($this->input->is_ajax_request()) {
        ...
        foreach ($list as $i) {
            ...
            // Cek apakah stok kurang dari 2 dan beri warna merah serta ikon danger
            if ($i->stok < 3) {
                $row[] = '<span style="color: red; font-weight: bold;">' . $i->stok . ' <i class="bi bi-exclamation-triangle-fill"></i></span>';
                $this->session->set_flashdata('ngok', 'Stok untuk barang <b> ' . $i->nama_barang . '</b> hampir habis. <b class="bi bi-exclamation-triangle-fill"></b>');
            } else {
                $row[] = $i->stok;
            }

            $row[] = '<span class="float-left">Rp.</span><span class="float-right">' . number_format($i->harga, 0, ',', '.') . ',-</span>';
            // Warna hijau untuk 'Aktif' dan merah untuk 'Tidak Aktif'
            if ($i->active == 'Y') {
                $row[] = '<span style="color: green; font-weight: bold;">Aktif</span>';
            } else {
                $row[] = '<span style="color: grey; font-weight: bold;">Tidak Aktif</span>';
            }
    ...
    public function ajax_stok_barang()
{
    ...
    if ($this->input->is_ajax_request()) {
        ...
        $data = array();
        $no = $_POST['start'];
        $stok_hampir_habis = false; // Menandakan apakah ada barang yang stoknya hampir habis
        ...
        foreach ($list as $i) {
            ...
            // Periksa jika stok kurang dari 2, jika ya, set flag stok_hampir_habis menjadi true
            if ($i->stok < 2) {
